<!-- <template>
	<view class="container">
		<tki-qrcode ref="qrcode" cid="tki-qrcode-canvas" val="二维码" :size="180" unit="px" background="#000000" foreground="#ffffff"
		 pdground="#ffffff" :icon="icon" :iconSize="40" :lv="3" :onval="false" :loadMake="false" :usingComponents="true"
...
<script>
	...
	export default {
		...
		data() {
			return {
				// 二维码
				icon: "http://lipinhui.28888753.cn/IntegralShop/backstage/Pay.jsp?get_userid=4id&isPay=2",
			}
		},
		methods: {
			// 二维码
					...
					<scroll-view class="specification-wrapper-content" scroll-y="true">
						...
						<view class="specification-content">
							<view class="specification-item" v-for="(item, index1) in specifications" :key="index1">
								...
								<view class="item-wrapper">
									<view class="item-content" v-for="(item_value, index2) in item.item" :key="index2"
							...
							<view class="specification-item">
								...
								<view class="item-wrapper">
									<stepper size="small" :min="1" :max="selectshop.stock" :defaultValue="selectNum"
										:display="canCount" @change="changeNum"></stepper>
								</view>
							</view>
						</view>
					</scroll-view>
	...
	export default {
		...
		methods: {
			skuClick(value, index1, event, index2) {
				...
				// value 第二层循环 ，index1（第一层循环下标），event当前对象 ，index2第二层循环下标
				if (value.ishow) {
					console.log("specifications", this.specifications)
			...
			checkItem() {
				...
				let result = this.difference.reduce(
					(arrs, items) => {
						console.log('初始值',arrs);
						console.log('当前元素',items);
						return arrs.concat(
			...
			checkInpath(clickIndex) {
				...
				for (let i = 0, len = this.specifications.length; i < len; i++) {
					...
					for (let j = 0; j < len2; j++) {
						// subIndex:判断是否为单选项 单选项则选中（-1多选项，反之为单选项）
						if (this.subIndex[i] != -1 && j == this.subIndex[i]) {
							continue;
						}
						// selectArr:存放被选中的值
						let choosed_copy = [...this.selectArr];
						//this.$set: 向响应式对象中添加一个 property，并确保这个新 property 同样是响应式的，
						//且触发视图更新。它必须用于向响应式对象上添加新 property，
						//因为 Vue 无法探测普通的新增 property 
						// (比如 this.myObject.newProperty = 'hi')
						// 参数：target(目标对象)，key(要更改的具体数据)，vlue（重新赋的值）
